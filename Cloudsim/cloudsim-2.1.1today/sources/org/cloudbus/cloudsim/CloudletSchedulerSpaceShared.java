...
public class CloudletSchedulerSpaceShared extends CloudletScheduler {
	 ...
	 */
	@Override
	public synchronized Cloudlet cloudletCancel(int cloudletId) {
		//First, looks in the finished queue
	//	System.out.println("Cancelling cloudllet  "+ cloudletId);
		...
		List<? extends ResCloudlet> cloudletList;
		cloudletList = getCloudletWaitingList();
		//Finally, looks in the waiting list
		for (ResCloudlet rcl : cloudletList) {
			//System.out.println("waiting cloudllet  "+ rcl.getCloudletId());
			if (rcl.getCloudletId() == cloudletId) {
				rcl.setCloudletStatus(Cloudlet.CANCELED);
			//	System.out.println("canceled cloudllet  "+ cloudletId);
		 		getCloudletWaitingList().remove(rcl);
				return rcl.getCloudlet();
			}
		}
	//	System.out.println("Ccouldnt cancel cloudllet  "+ cloudletId);
		return null;
	 ...
	 */
	@Override
	public synchronized double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		if ((currentCpus - usedPes) >= cloudlet.getPesNumber()) {// it can go to the exec list
			...
			rcl.setCloudletStatus(Cloudlet.QUEUED);
			getCloudletWaitingList().add(rcl);
		//	((List<ResCloudlet>) cloudletWaitingList).add(rcl);
	//		System.out.println("cloudlet with id  and vmid " + rcl.getCloudletId() +"  " + rcl.getCloudlet().getVmId() + " inserted into queue");
			return 0.0;
		}
