...
class myclass_datacenter extends Datacenter{
	...
	private DatacenterBroker1 datacenterBroker;
	...
	public void processEvent(SimEvent ev) {
        int srcId = -1;
      //  Log.printLine(CloudSim.clock()+"[Datacenter]: event received:"+ev.getTag());
	...
	public DatacenterBroker1 getBroker()
	{
		return this.datacenterBroker;
	}
	
	public void processPeriodicEvent(SimEvent ev)
	{
		...
		if(generatePeriodicEvent)
			send(getId(),1000,PERIODIC_EVENT,cloudletList);
		//if(totalDelay < 10000)
			//send(getId(), getSchedulingInterval(), CloudSimTags.CLOUDLET_SUBMIT);
	}
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
    	Vm vm = (Vm) ev.getData();
		 ...
		 sendNow(vm.getUserId(), DatacenterBroker1.VM_CREATED, data);
 	    }

 	    if (result) {
 	    	System.out.println("vm created with id = " + vm.getId());
			double amount = 0.0;
    ...
    protected void processVmDestroy(SimEvent ev, boolean ack) {
		Vm vm = (Vm) ev.getData();
		System.out.println("PROCESSVMDESTROY: VM id is " + vm.getId());
	...
	protected void processCloudletCancel(int cloudletId, int userId, int vmId) {
		Host h = getVmAllocationPolicy().getHost(vmId,userId);
		Vm vm = h.getVm(vmId,userId);
		//System.out.println(this.getName() + " : cancel " + cloudletId + "with vmid " + vmId);
		CloudletScheduler c = vm.getCloudletScheduler();
		Cloudlet cl = c.cloudletCancel(cloudletId);
