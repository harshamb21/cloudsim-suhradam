...
public class newcheck extends SimEntity {
	public static final int VM_CREATED = 56777;
	public static final int VM_CREATING = 67676;
	 ...
	 * @post $none
	 */
	public newcheck(String name) throws Exception {
		super(name);
	 ...
	 * @post $none
	 */
	private static int vmMax = 0,vmMin = 10000;
     ...
     */
		 int j = 0;
		 public void creatingVMs(SimEvent ev)
		 {
			 int negServiceTime = 3000;
			 Cloudlet ncloudlet = this.getCloudletList().get(0);
				...
				for(int i = 1; i <= nm; i++)
				{
					...
					nlist.add(vm2);
					System.out.println("fisrt:New VM " + i + " created");
					
					if(first) 	vmMax++;
					/*tempCounter++;
					if(tempCounter > vmMax)
					{
						vmMax = tempCounter;
					}*/
 ...
 private int m, oldm, min = 1, max = maxVMs; 
     ...
     * @post $none
     */
    private static int vmIndex;//,maxt=0;
    public static int NumberofVMs;
    private static int countervm2 = 0;// , counterDestroy =0;
    private static boolean first = true,firstDelete = false;
    private static int secMax,tempCounter = 0;
...
//**********************************Cloudlet submitted******************************************
			...
			if(numberVMscounter >= getVmList().size())
			{
				...
				if(this.getVmList().get(tempSize - 1).getvmQueue() == k)
				{
					...
					oldm = getVmList().size();
					int arrRate = cloudlets;						// 600 cloudlets per second		//lamda
				...
				//**************************RejRateCalculated*********************************					 
					//System.out.println("rejRate is " + rejRate + " reqExecTime is " + reqExecTime);
					if(rejRate > 0 || reqExecTime >= 250)
					{
						min = m + 1;
						m = m + m/2;						 
						if(m > max)
		...
		//System.out.println("beFOR: values of m and oldm are " + m + " " + oldm + " min and max " + min + " " + max + " k " + k);
		if(vm1.getvmQueue() < k/2)
		{
		//	System.out.println("FOR: values of m and oldm are " + m + " " + oldm);
			max = m;
			m = min + (max - min)/2;
			if(m <= min)
			{
		...
		int difference = 0;
		//********************** Destroying ***************************
		//List<Vm> vmDelet = new ArrayList<Vm>();
		...
		if(m < oldm)
		{
			...
			//	System.out.println("vmids for vm2 " + VList.get(y).getId() + " getVmList().get(y) " + getVmList().get(y).getId());
			}
					...
					for(Cloudlet cloudlet1 : CList1)
					{
						if(cloudlet1.getVmId() == vm1.getId())
						{
							...
							//	System.out.println("Value x:  " + x);
								...
								//System.out.println("vm1 " + vm1.getId() + " vm1size " + vm1.getvmQueue() + " vm2 " + vm2.getId() + " vm2size " + vm2.getvmQueue() + " pervm " + cloudletpervm);
								if(vm2.getvmQueue() < cloudletpervm)
								{
									countervm2 = (countervm2 + 1) % VList.size();
								//	System.out.println("Counter vm2 : " + countervm2);
									flag2 = true;
